...
void MoveNodeAway ()
{
	...
	Ptr<NetDevice> node_1 = devices.Get(0);
	Ptr<WifiNetDevice> mob_1 =  DynamicCast<WifiNetDevice> (node_1);

	mob_1->GetPhy()->SetAttribute ("TxPowerStart",DoubleValue(16));
	mob_1->GetPhy()->SetAttribute ("TxPowerEnd",DoubleValue(16));
...
int main (int argc, char *argv[])
{
	...
	/*positionAlloc->Add (Vector (0.0, 100.0, 0.0));
  	positionAlloc->Add (Vector (85.0, 100.0, 0.0));
	positionAlloc->Add (Vector (170.0, 100.0, 0.0));
	positionAlloc->Add (Vector (255.0, 100.0, 0.0));
	positionAlloc->Add (Vector (85.0, 0.0, 0.0));
	positionAlloc->Add (Vector (170.0, 200/.0, 0.0));*/
	...
	Time now = Simulator::Now ();
		//sSimulator::Schedule (Seconds (5), &MoveNodeAway);
