...
void MoveNodeAway ()
{
	...
	Ptr<Node> node_1 = nodes.Get (8);
  	...
  	mob_1->SetPosition(Vector (800.0, 0.0, 0.0));
	...
	Simulator::Schedule (Seconds (5.0 + xlp.GetValue()), &MoveNodeNear);
...
int main (int argc, char *argv[])
{
    ...
    positionAlloc->Add (Vector (800.0, 200.0, 0.0));
  	...
  	positionAlloc->Add (Vector (800.0, 100.0, 0.0));
  	positionAlloc->Add (Vector (900.0, 100.0, 0.0));
	...
	wifiPhy.Set("TxPowerStart",DoubleValue(15)); // 250-300 meter transmission range 
  	wifiPhy.Set("TxPowerEnd",DoubleValue(15));   // 250-300 meter transmission range 
	...
	Time now = Simulator::Now ();
	Simulator::Schedule (Seconds (10), &MoveNodeAway);
