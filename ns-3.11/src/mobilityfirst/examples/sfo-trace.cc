...
{
	int nNodes = 1; // change when number of nodes change
	int nFlows = 0; // number of flows

	//GlobalValue::Bind ("SimulatorImplementationType", StringValue ("ns3::RealtimeSimulatorImpl"));
	double tag = atof(argv[2]);

	cout << "TAG: " << tag << endl;

	LogComponentEnable("SimplePointToPointGstarExample",LOG_LEVEL_ALL);
	Config::SetDefault ("ns3::mf::RoutingProtocol::ThruputInterval", TimeValue (Seconds(1/tag)));
	...
	Ns2MobilityHelper mobility = Ns2MobilityHelper("one-node.txt");
	mobility.Install();
	
	/*ObjectFactory pos;
	pos.SetTypeId ("ns3::RandomRectanglePositionAllocator");
	pos.Set ("X", RandomVariableValue (UniformVariable (0.0, 200.0)));
	pos.Set ("Y", RandomVariableValue (UniformVariable (0.0, 200.0)));
