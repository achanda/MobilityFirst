...
TypeId
BypassSocketImpl::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BypassSocketImpl")
    .SetParent<BypassSocket> ()
    .AddConstructor<BypassSocketImpl> ()
    .AddTraceSource ("Drop", "Drop UDP packet due to receive buffer overflow",
                     MakeTraceSourceAccessor (&BypassSocketImpl::m_dropTrace))
    .AddAttribute ("IcmpCallback", "Callback invoked whenever an icmp error is received on this socket.",
                   CallbackValue (),
                   MakeCallbackAccessor (&BypassSocketImpl::m_icmpCallback),
                   MakeCallbackChecker ())
  ;
  return tid;
}

BypassSocketImpl::BypassSocketImpl ()
  : m_endPoint (0),
    m_node (0),
    m_udp (0),
...
void 
BypassSocketImpl::SetNode (Ptr<Node> node)
{
  m_node = node;
}
void 
BypassSocketImpl::SetUdp (Ptr<BypassProtocol> udp)
{
  m_udp = udp;
}
...
BypassSocketImpl::GetErrno (void) const
{
  return m_errno;
}

enum Socket::SocketType
BypassSocketImpl::GetSocketType (void) const
{
  return NS3_SOCK_DGRAM;
}

Ptr<Node>
BypassSocketImpl::GetNode (void) const
{
  return m_node;
}

void 
BypassSocketImpl::Destroy (void)
{
  m_node = 0;
  m_endPoint = 0;
  m_udp = 0;
...
int
BypassSocketImpl::FinishBind (void)
{
  if (m_endPoint == 0)
    {
      return -1;
    }
  m_endPoint->SetRxCallback (MakeCallback (&BypassSocketImpl::ForwardUp, Ptr<BypassSocketImpl> (this)));
  m_endPoint->SetIcmpCallback (MakeCallback (&BypassSocketImpl::ForwardIcmp, Ptr<BypassSocketImpl> (this)));
  m_endPoint->SetDestroyCallback (MakeCallback (&BypassSocketImpl::Destroy, Ptr<BypassSocketImpl> (this)));
  return 0;
}

int
BypassSocketImpl::Bind (void)
{
...
int 
BypassSocketImpl::Bind (const Address &address)
{
...
int 
BypassSocketImpl::ShutdownSend (void)
{
  m_shutdownSend = true;
  return 0;
}
...
int 
BypassSocketImpl::ShutdownRecv (void)
{
  m_shutdownRecv = true;
  return 0;
}
...
int
BypassSocketImpl::Connect (const Address & address)
{
  //NS_LOG_FUNCTION (this << address);
  InetSocketAddress transport = InetSocketAddress::ConvertFrom (address);
...
int 
BypassSocketImpl::Send (Ptr<Packet> p, uint32_t flags)
{
...
int 
BypassSocketImpl::DoSend (Ptr<Packet> p)
{
...
int
BypassSocketImpl::DoSendTo (Ptr<Packet> p, const Address &address)
{
...
int
BypassSocketImpl::DoSendTo (Ptr<Packet> p, Ipv4Address dest, uint16_t port)
{
  ...
  if (dest.IsBroadcast ())
  ...
  else if (ipv4->GetRoutingProtocol () != 0)
    {
      ...
      header.SetProtocol (BypassProtocol::PROT_NUMBER);
...
uint32_t
BypassSocketImpl::GetTxAvailable (void) const
{
...
int 
BypassSocketImpl::SendTo (Ptr<Packet> p, uint32_t flags, const Address &address)
{
...
uint32_t
BypassSocketImpl::GetRxAvailable (void) const
{
...
Ptr<Packet>
BypassSocketImpl::Recv (uint32_t maxSize, uint32_t flags)
{
...
Ptr<Packet>
BypassSocketImpl::RecvFrom (uint32_t maxSize, uint32_t flags, 
                         Address &fromAddress)
{
  //NS_LOG_FUNCTION (this << maxSize << flags);
...
int
BypassSocketImpl::GetSockName (Address &address) const
{
...
int 
BypassSocketImpl::MulticastJoinGroup (uint32_t interface, const Address &groupAddress)
{
...
int 
BypassSocketImpl::MulticastLeaveGroup (uint32_t interface, const Address &groupAddress) 
{
  //NS_LOG_FUNCTION (interface << groupAddress);
  /*
   1) sanity check interface
...
void
BypassSocketImpl::BindToNetDevice (Ptr<NetDevice> netdevice)
{
...
void 
BypassSocketImpl::ForwardUp (Ptr<Packet> packet, Ipv4Header header, uint16_t port,
...
void
BypassSocketImpl::ForwardIcmp (Ipv4Address icmpSource, uint8_t icmpTtl, 
...
void 
BypassSocketImpl::SetRcvBufSize (uint32_t size)
{
  m_rcvBufSize = size;
}

uint32_t 
BypassSocketImpl::GetRcvBufSize (void) const
{
  return m_rcvBufSize;
}

void 
BypassSocketImpl::SetIpTtl (uint8_t ipTtl)
{
  m_ipTtl = ipTtl;
}

uint8_t 
BypassSocketImpl::GetIpTtl (void) const
{
  return m_ipTtl;
}

void 
BypassSocketImpl::SetIpMulticastTtl (uint8_t ipTtl)
{
  m_ipMulticastTtl = ipTtl;
}

uint8_t 
BypassSocketImpl::GetIpMulticastTtl (void) const
{
  return m_ipMulticastTtl;
}

void 
BypassSocketImpl::SetIpMulticastIf (int32_t ipIf)
{
  m_ipMulticastIf = ipIf;
}

int32_t 
BypassSocketImpl::GetIpMulticastIf (void) const
{
  return m_ipMulticastIf;
}

void 
BypassSocketImpl::SetIpMulticastLoop (bool loop)
{
  m_ipMulticastLoop = loop;
}

bool 
BypassSocketImpl::GetIpMulticastLoop (void) const
{
  return m_ipMulticastLoop;
}

void 
BypassSocketImpl::SetMtuDiscover (bool discover)
{
  m_mtuDiscover = discover;
}
bool 
BypassSocketImpl::GetMtuDiscover (void) const
{
  return m_mtuDiscover;
}

bool
BypassSocketImpl::SetAllowBroadcast (bool allowBroadcast)
{
...
bool
BypassSocketImpl::GetAllowBroadcast () const
{
